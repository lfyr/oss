  ...
  <div class="body-content">
    <form method="post" class="form-x" action="<?php echo U('edit');?>">  
      ...
      <div class="form-group">
          ...
          <?php foreach($roleData as $k=>$v): if(strpos(','.$role_id['role_id'].',',','.$v['id'].',') !==FALSE){ $check='checked="checked"'; }else{ $check=''; } ?>
            <input <?php echo ($check); ?> type="checkbox" id="ishome"  name="role_id[]" value="<?php echo ($v["id"]); ?>"/><?php echo ($v["role_name"]); ?>
          <?php endforeach;?>
          </div>
      </div>
    <?php endif;?>
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" value="<?php echo ($data["username"]); ?>" name="username" data-validate="required:请输用户名" />
          <div class="tips"></div>
        </div>
      ...
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" value="<?php echo ($data["bucket"]); ?>" name="bucket" data-validate="required:请输入所属库" />
          <div class="tips"></div>
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" value="<?php echo ($data["root_dir"]); ?>" name="root_dir" data-validate="required:请输入根目录" />
          <div class="tips"></div>
        </div>
      </div>
      <?php endif;?>
