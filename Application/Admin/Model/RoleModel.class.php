...
class RoleModel extends Model{
	...
	public function search($pageSize = 2)
	{
		...
		$data['page'] = $page->show();
		#select a.*,GROUP_CONCAT(c.pri_name) from oss_role a left join oss_role_privilege b on a.id=b.role_id left join oss_privilege c on b.pri_id=c.id group by a.id;
		$data['data'] = $this->field('a.*,GROUP_CONCAT(c.pri_name) pri_name')->alias('a')->join('left join oss_role_privilege b on a.id=b.role_id left join oss_privilege c on b.pri_id=c.id')->where($where)->group('a.id')->limit($page->firstRow.','.$page->listRows)->select();
		return $data;
	}
}